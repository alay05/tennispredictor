...
| 02-01-02 | 01 | 1 | FEAT-01 | T-02-01-01 / T-02-01-03 | Runner emits pre-match snapshots before updates and derives minimal ranking differentials from snapshot contracts only, with `FeatureDifferentialRow` carrying `feature_version`, `canonical_match_id`, `player_a_id`, `player_b_id`, `as_of_date`, and explicit A/B orientation metadata. | unit | `./.venv/bin/python -m pytest -q tests/unit/test_feature_runner.py tests/unit/test_feature_rankings.py tests/unit/test_feature_ordering.py` | ✅ after 02-01-01 | ⬜ pending |
...
| 02-04-01 | 04 | 4 | FEAT-09 | T-02-04-01 / T-02-04-03 | Red tests prove persistence and family-specific leakage invariants for ranking, `ranking_change`, Elo, recent-form, H2H, and serve/return aggregates are still missing before implementation. | unit-red | `! ./.venv/bin/python -m pytest -q tests/unit/test_feature_leakage.py tests/unit/test_feature_persistence.py` | ❌ W0 | ⬜ pending |
| 02-04-02 | 04 | 4 | FEAT-08 | T-02-04-01 / T-02-04-02 / T-02-04-03 | Persistence writes the full FEAT-08 differential contract with `feature_version`, `canonical_match_id`, `player_a_id`, `player_b_id`, `as_of_date`, and orientation fields, and historical outputs stay invariant across ranking, `ranking_change`, Elo, recent-form, H2H, and serve/return aggregates under future-row deletion and same-cohort reordering. | unit | `./.venv/bin/python -m pytest -q tests/unit/test_feature_leakage.py tests/unit/test_feature_persistence.py` | ✅ after 02-04-01 | ⬜ pending |
